...
import type { mcpServers } from '../../shared/lib/recordSchemas';
...
export class McpClientManager {
  ...
  private async createConnection(serverId: number): Promise<McpConnection> {
    const serverConfig = await db.mcpServers.findUnique({
      where: { id: serverId }
    });
  ...
  async getAllEnabledTools(): Promise<McpTool[]> {
    const enabledServers = await db.mcpServers.findMany({
