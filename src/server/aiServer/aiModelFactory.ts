...
import { upsertPlanIncTool } from './tools/createPlaninc';
...
import { searchPlanIncTool } from './tools/searchPlaninc';
import { updatePlanIncTool } from './tools/updatePlaninc';
import { deletePlanIncTool } from './tools/deletePlaninc';
...
export class AiModelFactory {
  ...
  static async rebuildVectorIndex({ vectorStore, isDelete = false }: { vectorStore: LibSQLVector; isDelete?: boolean }) {
    try {
      if (isDelete) {
        await vectorStore.deleteIndex({ indexName: 'planinc' });
      }
    } catch (error) {
  ...
  static async getAiProvider(id: number) {
    return await db.aiProviders.findUnique({
      where: { id },
      include: { models: true }
    });
  }
  ...
  static async getAllAiProviders() {
    return await db.aiProviders.findMany({
      include: { models: true },
  ...
  static async getAiModel(id: number) {
    return await db.aiModels.findUnique({
      where: { id },
  ...
  static async getAiModelsByCapability(capability: string) {
    return await db.aiModels.findMany({
      where: {
        capabilities: {
          path: [capability],
          equals: true
  ...
  static async BaseChatAgent({ withTools = true, withOnlineSearch = false, withMcpTools = true, extraInstructions }: { withTools?: boolean; withOnlineSearch?: boolean; withMcpTools?: boolean; extraInstructions?: string }) {
    ...
    const defaultInstructions =
      ...
      "7. When using 'search-planinc-tool', The entire content of the note should not be returned unless specifically specified by the user " +
    ...
    const PlanIncAgent = new Agent({
      name: 'PlanInc Chat Agent',
      instructions,
      model: provider?.LLM!,
      ...tools,
    });
    ...
    return mastra.getAgent('PlanIncAgent');
  }
  ...
  static CommentAgent = AiModelFactory.#createAgentFactory(
    'PlanInc Comment Agent',
    `You are PlanInc Comment Assistant. Guidelines:
  ...
  static TestConnectAgent = AiModelFactory.#createAgentFactory('PlanInc Test Connect Agent', `Test the api is working,return 1 words`, 'PlanIncTestConnect');

  static ImageEmbeddingAgent = AiModelFactory.#createAgentFactory(
    'PlanInc Image Embedding Agent',
