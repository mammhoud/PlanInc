 ...
 *
 * The `deleteMany` attachments endpoint used to call `db.attachments.deleteMany()`
 ...
 *
 * The heavy modules (`../middleware`, `../db`, `../lib/files`) are mocked so
 * we can exercise the real resolver in `routerTrpc/attachment.ts` without
 * standing up tRPC, the SurrealDB data layer, or any storage backend.
 */
...
// --- mock db: findMany returns the owned attachments, deleteMany must NOT be used
const findMany = mock((_args: any) => Promise.resolve([] as any[]));
const deleteMany = mock((_args: any) => Promise.resolve({ count: 0 }));
mock.module('../../../db', () => ({
  db: { attachments: { findMany, deleteMany } },
}));
