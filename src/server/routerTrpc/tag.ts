...
import { tagSchema } from '@shared/lib/recordSchemas';
...
export const tagRouter = router({
  ...
  fullTagNameById: authProcedure
    ...
    .query(async function ({ input }) {
      ...
      const getParentTags = async (currentTag: typeof tag): Promise<string[]> => {
        ...
        const parentTag = await db.tag.findFirst({
  ...
  updateTagMany: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      const { ids, tag } = input
      const notes = await db.notes.findMany({ where: { id: { in: ids } } })
  ...
  updateTagName: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      const tagToNote = await db.tagsToNote.findMany({ where: { tagId: id } })
      const noteIds = tagToNote.map(i => i.noteId)
      const hasTagNote = await db.notes.findMany({ where: { id: { in: noteIds } } })
      hasTagNote.map(i => {
  ...
  updateTagIcon: authProcedure
    ...
    .mutation(async function ({ input }) {
      const { id, icon } = input
      return await db.tag.update({ where: { id }, data: { icon } })
    }),
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      for (const noteId of allNotesId) {
        ...
        const getAllTagIdsInChain = async (tagId: number): Promise<number[]> => {
          ...
          let currentTag = await db.tag.findFirst({ where: { id: tagId } });
          while (currentTag && currentTag.parent !== 0) {
            result.push(currentTag.parent);
            currentTag = await db.tag.findFirst({ where: { id: currentTag.parent } });
          }
          
          const childTags = await db.tag.findMany({ where: { parent: tagId } });
        ...
        for (const tagId of tagIdsInChain) {
          ...
          if (tagExists) {
            const tagUsageCount = await db.tagsToNote.count({
              where: { tagId }
            });
  ...
  deleteTagWithAllNote: authProcedure.use(demoAuthMiddleware)
    ...
    .mutation(async function ({ input, ctx }) {
      const { id } = input
      const tag = await db.tag.findFirst({ where: { id, accountId: Number(ctx.id) }, include: { tagsToNote: true } })
      const allNotesId = tag?.tagsToNote.map(i => i.noteId) ?? []
