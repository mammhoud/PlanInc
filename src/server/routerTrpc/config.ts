...
import { configSchema } from '@shared/lib/recordSchemas';
...
export const getAiModelConfig = async (type: 'mainModel' | 'embeddingModel' | 'voiceModel' | 'rerankModel' | 'imageModel' | 'audioModel', ctx?: Context) => {
  ...
  // Get the model with provider information directly from db
  const model = await db.aiModels.findUnique({
    where: { id: modelId },
...
export const configRouter = router({
  ...
  update: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      if (isUserPreferConfig) {
        const matchedConfigs = await db.config.findMany({ where: { userId, key } });
        
        if (matchedConfigs.length > 0) {
          const configToKeep = matchedConfigs[0];
          updateResult = await db.config.update({ 
          ...
          updateResult = await db.config.create({ data: { userId, key, config: { type: typeof value, value } } });
        }
      } else {
        if (ctx.role !== 'superadmin') {
          throw new Error('You are not allowed to update global config')
        }
        const matchedConfigs = await db.config.findMany({ where: { key } });
        ...
        if (matchedConfigs.length > 0) {
          ...
          updateResult = await db.config.create({ data: { key, config: { type: typeof value, value } } });
        }
      }
  ...
  setPluginConfig: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      const hasKey = await db.config.findFirst({ where: { userId, key: `plugin_config_${pluginName}_${key}` } })
      if (hasKey) {
        return await db.config.update({ where: { id: hasKey.id }, data: { config: { type: typeof value, value } } })
      }
      return await db.config.create({ data: { userId, key: `plugin_config_${pluginName}_${key}`, config: { type: typeof value, value } } })
    }),
