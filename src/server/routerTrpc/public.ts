  ...
  latestClientVersion: publicProcedure
    ...
    .query(async function () {
      return await cache.wrap(
        ...
        async () => {
          const url = `https://api.github.com/repos/mammhoud/planinc/releases/latest`;
          try {
  ...
  latestServerVersion: publicProcedure
    ...
    .query(async function () {
      return await cache.wrap(
        ...
        async () => {
          try {
            const url = 'https://hub.docker.com/v2/repositories/mammhoud/planinc/tags';
            const res = await getWithProxy(url, {
              config: {
                headers: {
      ...
      return cache.wrap(
        ...
        async () => {
          if (!input?.id || input?.id === null) {
            const superAdmin = await db.accounts.findFirst({ where: { role: 'superadmin' } });
            return {
          ...
          const account = await db.accounts.findFirst({ where: { id: Number(input?.id) } });
          return {
    ...
    .query(async function ({ input }) {
      ...
      return await cache.wrap(
        ...
        async () => {
          try {
            //raw.gitmirror.com
            const response = await getWithProxy('https://raw.githubusercontent.com/mammhoud/planinc-hub/refs/heads/main/index.json', {
              config: {
                headers: {
