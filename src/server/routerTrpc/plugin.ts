...
import { pluginSchema } from '@shared/lib/recordSchemas';
...
const ensurePluginDir = async () => {
  const dir = path.join(process.cwd(), '.planinc', 'plugins');
...
const getPluginDir = (pluginName: string) => {
  return path.join('.planinc', 'plugins', pluginName);
};
...
export const pluginRouter = router({
  getAllPlugins: authProcedure
    .query(async () => {
      return cache.wrap(
        ...
        async () => {
          try {
            const response = await getWithProxy('https://raw.githubusercontent.com/mammhoud/planinc-plugin-marketplace/main/index.json');
  ...
  getPluginCssContents: authProcedure
    ...
    .query(async ({ input }) => {
      try {
        const pluginDir = path.join('.planinc', 'plugins', input.pluginName);
