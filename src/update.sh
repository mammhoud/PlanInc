...
container_name="planinc-website"
image_name="mammhoud/planinc:latest"
backup_dir="planinc-backup-$(date +"%Y%m%d_%H%M%S")"
docker_volume=""
...
if [ "$(docker ps -q -f name=$container_name)" ]; then
  docker cp "${container_name}:/app/.planinc" "$backup_dir"
...
# Step 2: Remove existing mammhoud/planinc Docker image
echo -e "${YELLOW}1. 🗑 Removing existing mammhoud/planinc Docker image...${NC}"
docker rmi "$image_name" -f

# Check if the container has a volume mounted
volume_path=$(docker inspect -f '{{ range .Mounts }}{{ if eq .Destination "/app/.planinc" }}{{ .Source }}{{ end }}{{ end }}' "$container_name")
if [ -z "$volume_path" ]; then
  ...
  docker_volume="-v $volume_path:/app/.planinc"
fi
...
# Step 3: Run the planinc-website container
echo -e "${YELLOW}2. ⏳ Starting the planinc-website container...${NC}"
