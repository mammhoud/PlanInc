  ...
  return (
    <div className={`flex items-center select-none ${isExpanded ? 'mb-4' : 'mb-1'}`}>
      <div className={`flex items-center w-full gap-1 ${isExpanded ? 'text-base' : 'text-xs'}`}>
        ...
        {!isShareMode && (
          <ShareButton planincItem={planincItem} isIOSDevice={isIOSDevice} />
        )}
        ...
        {!isShareMode && (
          <Tooltip content={t('trash')} delay={1000}>
            <Icon
              ...
              width={iconSize}
              height={iconSize}
              className={`opacity-0 group-hover/card:opacity-100 group-hover/card:translate-x-0 ml-2 cursor-pointer hover:text-red-500 text-desc ${planincItem.isRecycle ? 'text-red-500 opacity-100' : ''}`}
              onClick={(e) => {
                e.stopPropagation();
...
const ShareButton = observer(({ planincItem, isIOSDevice }: { planincItem: Note, isIOSDevice: boolean }) => {
  const { t } = useTranslation()
