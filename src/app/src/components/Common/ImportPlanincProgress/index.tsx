...
import { type RestoreResult } from '@shared/lib/types'
...
export const ImportProgress = observer(({ filePath }: { filePath: string }) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    handleAsyncGenerator: async () => {
      const asyncGeneratorRes = await streamApi.task.importFromPlanInc.mutate({ filePath })
...
export const ShowPlanIncProgressDialog = async (filePath: string) => {
