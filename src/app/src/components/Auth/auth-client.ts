...
export async function getTokenData(): Promise<TokenData | null> {
  try {
    ...
    const response = await fetch(getPlanIncEndpoint('/api/auth/profile'), {
      headers: {
...
export async function signIn(
  ...
  try {
    if (provider === 'credentials') {
      console.log('signIn Endpoint', getPlanIncEndpoint('/api/auth/login'));
      const response = await fetch(getPlanIncEndpoint('/api/auth/login'), {
        method: 'POST',
        headers: { 
    ...
    if (provider === 'oauth-2fa') {
      const response = await fetch(getPlanIncEndpoint('/api/auth/verify-2fa'), {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
        },
...
export async function signOut(options: { redirect?: boolean; callbackUrl?: string } = {}): Promise<{ url: string }> {
  try {
    ...
    await fetch(getPlanIncEndpoint('/api/auth/logout'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
