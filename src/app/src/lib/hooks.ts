...
export const useConfigSetting = (configKey: keyof PlanIncStore['config']['value']) => {
  ...
  useEffect(() => {
    if (planinc.config.value && planinc.config.value[configKey]) {
      store.setValue(planinc.config.value[configKey] as string);
    }
  }, [planinc.config.value, configKey]);
...
const initializeAndroidShortcuts = () => {
  ...
  const checkAndroidData = () => {
      ...
      if (action) {
        ...
        switch (action) {
          case 'quick_note':
            ShowEditPlanIncModel('2xl', 'create');
      ...
      if (shareDataStr && !isProcessingSharedData) {
        ...
        try {
          ...
          if (shareData.text) {
            ...
            ShowEditPlanIncModel('2xl', 'create', { text: cleanText });
            ...
            readFile(shareData.stream).then(contents => {
              ...
              console.log('xxx!!!')
              ShowEditPlanIncModel('2xl', 'create', { file });
              isProcessingSharedData = false;
            }).catch((error: Error) => {
