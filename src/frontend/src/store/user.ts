...
export class UserStore implements Store {
  ...
  tokenData = new StorageState<TokenData | null>({ key: 'planincToken', value: null });
  theme: any = 'light';
  ...
  get planinc() {
    return RootStore.Get(PlanIncStore);
  }
  ...
  canRegister = new PromiseState({
    function: async () => {
      if (isInTauri() && getPlanIncEndpoint() == '') {
        return
      }
  ...
  updateAppTitle(customTitle?: string) {
    const appTitle = customTitle?.trim() || 'PlanInc';
  ...
  async initializeSettings(setTheme: (theme: string) => void, i18n: any) {
    ...
    const config = await this.planinc.config.call()
    ...
    const handleFeatureRoute = (
      ...
      const configKey = featureKey === 'ai' ? 'mainModelId' : `isUsePlanIncHub`;
    ...
    if (config?.themeColor && config?.themeForegroundColor) {
      applyThemePalette(config.themeColor, config.themeForegroundColor)
    } else {
      ...
      applyThemePalette(DEFAULT_THEME_PALETTE.background, DEFAULT_THEME_PALETTE.foreground)
    }
    ...
    // Appearance v2 (PI-011 · P3): density, interface scale, line height, text
    // direction, reduced motion and contrast boost are applied to <html> as
    // attributes plus one custom property — see lib/appearance.ts and the
  ...
  use() {
    ...
    useEffect(() => {
      this.updateAppTitle(this.planinc.config.value?.customTitle);
    }, [this.planinc.config.value?.customTitle]);
