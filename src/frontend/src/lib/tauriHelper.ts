...
import { setStatusBarColor } from 'tauri-plugin-planinc-api'
    ...
    try {
        ...
        if (isAndroid()) {
            ...
            if (shouldShowSettings) {
                try {
                    const { openAppSettings } = await import('tauri-plugin-planinc-api');
                    await openAppSettings();
                } catch (error) {
                    console.error('Failed to open app settings:', error);
                    // Fallback: Show instructions
                    alert('Please go to Settings > Apps > PlanInc > Permissions and enable Microphone access.');
                }
            }
