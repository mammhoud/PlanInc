...
export function PlanincGraph({ nodes, edges, selectedNode, hoveredNode, labels, relationCount, onSelect, onHover, ariaLabel, storageKey = 'planinc:graph:camera' }: Props) {
  ...
  return <div className="overflow-hidden rounded-2xl bg-[radial-gradient(circle_at_top,#e0f2fe,transparent_60%),#f0f9ff] p-2 dark:bg-[radial-gradient(circle_at_top,#0c4a6e,transparent_60%),#082f49]">
    <svg ref={svgRef} viewBox="0 0 800 500" className="min-h-[420px] min-w-[640px] w-full select-none touch-none" role="img" aria-label={ariaLabel}>
      <style>{`.graph-node-halo{fill:#38bdf8;filter:blur(4px)}.graph-node-label{pointer-events:none;font-weight:600}.graph-node-dot{vector-effect:non-scaling-stroke}`}</style>
    </svg>
    <div className="flex items-center justify-between gap-2 px-2 pb-1 pt-2 text-tiny text-sky-700 dark:text-sky-200">
      ...
      <span className="flex gap-1">
        <button type="button" className="rounded-md px-2 py-1 hover:bg-sky-200/60 dark:hover:bg-sky-800/60" onClick={() => updateCamera('fit')}>Fit</button>
        <button type="button" className="rounded-md px-2 py-1 hover:bg-sky-200/60 dark:hover:bg-sky-800/60" onClick={() => updateCamera('reset')}>Reset</button>
      </span>
    </div>
