  ...
  const store = RootStore.Local(() => ({
    deleteFile: new PromiseState({
      function: async (file) => {
        ...
        if (path) {
          await axiosInstance.post(getPlanIncEndpoint('/api/file/delete'), {
        ...
        RootStore.Get(PlanIncStore).removeCreateAttachments(file)
      }
    })
  }))
...
export const InsertConextButton = observer(({ className, file, files, size = 20 }: { className: string, file: FileType, files: FileType[], size?: number }) => {
  ...
  return <>
    <Tooltip>
      ...
      <TooltipContent>{t('insert-context')}</TooltipContent>
    </Tooltip>
  </>
})

export const DownloadIcon = observer(({ className, file, size = 20 }: { className?: string, file: FileType, size?: number }) => {
  // Was `hidden group-hover:block`: display:none until hover, which meant an
  // attachment could not be downloaded at all on a phone or tablet. The reveal
  // is now `.hover-only-on-fine` (opacity, driven by the pointer capability) so
  // a coarse pointer always sees it and a mouse still gets the reveal.
  return <div className={`hover-only-on-fine p-1 !transition-all absolute z-10 right-[5px] top-[5px] !text-background !bg-foreground cursor-pointer rounded-sm ${className}`}>
    <Icon onClick={() => {
      downloadFromLink(getPlanIncEndpoint(file.uploadPromise.value));
...
export const CopyIcon = observer(({ className, file, size = 20 }: { className?: string, file: FileType, size?: number }) => {
  ...
  const copyImageToClipboard = async () => {
    try {
      ...
      const response = await axiosInstance.get(getPlanIncEndpoint(src), {
        responseType: 'blob'
      });
        ...
        img.onload = async () => {
          ...
          canvas.toBlob(async (blob) => {
            ...
            try {
              ...
              try {
                await navigator.clipboard.writeText(getPlanIncEndpoint(src));
                RootStore.Get(ToastPlugin).success(t('operation-success'));
                resolve(true);
              } catch (textError) {
  ...
  // Same fix as DownloadIcon above: `hidden group-hover:block` was unreachable
  // without a mouse.
  return <div className={`hover-only-on-fine p-1 !transition-all absolute z-10 right-[30px] top-[5px] !text-background !bg-foreground cursor-pointer rounded-sm ${className}`}>
