...
const renderAllVditorContent = (
  ...
  const cdn = getPlanIncEndpoint('').replace(/\/$/, "");
  const styleName = getHighlightStyle(theme);
...
export const useEditorInit = (
  ...
  useEffect(() => {
    ...
    const vditor = new Vditor("vditor" + "-" + mode, {
      ...
      upload: {
        url: getPlanIncEndpoint('/api/file/upload'),
        success: (editor, res) => {
        ...
        fieldName: 'file',
        multiple: false,
        linkToImgUrl: getPlanIncEndpoint('/api/file/upload-by-url'),
        linkToImgFormat(res) {
  ...
  }, [mode, planinc.config.value?.toolbarVisibility, store.viewMode, isPc]);
  ...
  useEffect(() => {
    if (mode == 'create') {
      ...
      store.noteType = toNoteTypeEnum(planinc.curSelectedNote?.type)
    }
