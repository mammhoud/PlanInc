...
export class EditorStore {
  ...
  private _viewMode: ViewMode = (() => {
    try {
      const saved = localStorage.getItem('planinc-editor-view-mode');
  ...
  set viewMode(mode: ViewMode) {
    this._viewMode = mode;
    try {
      localStorage.setItem('planinc-editor-view-mode', mode);
    } catch (error) {
  ...
  get showIsEditText() {
    if (this.mode == 'edit') {
      try {
        const local = this.planinc.editContentStorage.list?.find(i => Number(i.id) == Number(this.planinc.curSelectedNote?.id))
  ...
  uploadFiles = async (acceptedFiles) => {
    ...
    if (this.mode == 'create') {
      _acceptedFiles.map(i => ({
        ...
        RootStore.Get(PlanIncStore).createAttachmentsStorage.push(t)
      })
    } else {
      _acceptedFiles.map(i => ({
        name: i.name,
        ...
        type: uploadFileType?.[i.name],
        size: i.size,
        id: this.planinc.curSelectedNote?.id!
      })).map(t => {
        RootStore.Get(PlanIncStore).editAttachmentsStorage.push(t)
      })
    }
  }
  ...
  isShowEditorToolbar(isPc: boolean) {
    ...
    if (planinc.config.value?.toolbarVisibility) {
      showToolbar = planinc.config.value?.toolbarVisibility == 'always-show-toolbar' ? true : (
        planinc.config.value?.toolbarVisibility == 'hide-toolbar-on-mobile' ?
          (isPc ? true : false)
          : false
      )
    }
