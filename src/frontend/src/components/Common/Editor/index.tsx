...
const Editor = observer(({ content, onChange, onSend, isSendLoading, originFiles, originReference = [], mode, onHeightChange, hiddenToolbar = false, withoutOutline = false, initialData, showTopToolbar = false }: IProps) => {
  ...
  useEffect(() => {
    if (showTopToolbar) {
      ...
      const noteSpecificId = `editor-top-toolbar-${planinc.curSelectedNote?.id}`;
  ...
  }, [showTopToolbar, planinc.curSelectedNote?.id]);
  ...
  useEditorFiles(store, planinc, originFiles);
  useEditorHeight(onHeightChange, planinc, content, store);
