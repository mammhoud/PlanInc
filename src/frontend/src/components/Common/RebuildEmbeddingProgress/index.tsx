...
export const ImportProgress = observer(({ force }: { force: boolean }) => {
  ...
  const getStatusIcon = (type: string) => {
    switch (type) {
      case 'success':
        return <Icon icon="mingcute:check-circle-fill" className="text-success" width={16} height={16} />;
      case 'error':
        return <Icon icon="mingcute:close-circle-fill" className="text-destructive" width={16} height={16} />;
      case 'skip':
        return <Icon icon="mingcute:refresh-3-line" className="text-warning" width={16} height={16} />;
      case 'info':
        return <Icon icon="mingcute:information-fill" className="text-info" width={16} height={16} />;
      default:
        return <Icon icon="mingcute:dot-fill" className="text-ignore" width={16} height={16} />;
    }
  };
  ...
  return <div className="space-y-4">
      ...
      <div className="flex justify-end items-center">{/* 移除了 justify-between，只保留右对齐 */}
        ...
        {store.status === 'running' && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-destructive-soft hover:bg-destructive/10 dark:bg-destructive-soft/50 dark:hover:bg-destructive-soft/70 text-destructive rounded-md transition-all duration-200 text-sm font-medium"
            title={t('stop-task')}
          >
        ...
        {store.isSuccess && (
          <div className="flex items-center gap-2 px-4 py-2 bg-success-soft dark:bg-success-soft/50 text-success rounded-full text-sm font-medium">
        ...
        {store.isError && (
          <div className="flex items-center gap-2 px-4 py-2 bg-destructive-soft dark:bg-destructive-soft/50 text-destructive rounded-full text-sm font-medium">
    ...
    <div className='flex flex-col max-h-[400px] overflow-y-auto mt-4 space-y-2 pr-2 scrollbar-thin scrollbar-thumb-border dark:scrollbar-thumb-desc scrollbar-track-transparent'>
      {store.message.map((item, index) => (
        <div key={index} className={`flex gap-3 p-4 rounded-lg items-start transition-all duration-200 ${item.type === 'error' ? 'bg-destructive-soft dark:bg-destructive-soft/20' :
            item.type === 'success' ? 'bg-success-soft dark:bg-success-soft/20' :
              item.type === 'info' ? 'bg-info-soft dark:bg-info-soft/20' :
                'bg-secondbackground dark:bg-muted/50'
          }`}>
          <div className="flex-shrink-0 mt-0.5">
            {getStatusIcon(item.type)}
          </div>
          <div className='flex flex-col flex-1 min-w-0'>
            <div className={`text-sm font-medium ${item.type === 'error' ? 'text-destructive-soft-foreground' :
                item.type === 'success' ? 'text-success-soft-foreground' :
                  item.type === 'info' ? 'text-info-soft-foreground' :
                    'text-foreground'
              }`}>
              {item?.content}
            </div>
            {item.error as unknown as string && (
              <div className="mt-2 p-3 bg-destructive/10 dark:bg-destructive-soft/30 rounded-lg text-xs text-destructive">
      ...
      {store.message.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-ignore">
