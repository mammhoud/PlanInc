...
export const RegistrySection = observer(({
  ...
  return (
    <div className="flex min-w-0 flex-col">
      {meta && <div className="font-semibold mt-2 min-w-0 break-words">{t(meta.labelKey)}</div>}
      {groups.map(({ group, items }) => (
        <div key={`${section}:${group}`} className="flex min-w-0 flex-col">
          <div className="text-xs uppercase tracking-wide text-ignore mt-2 min-w-0 truncate">{t(group)}</div>
          {items.map((setting) => (
