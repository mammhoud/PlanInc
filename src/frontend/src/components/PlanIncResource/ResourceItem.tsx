...
export const ResourceItemPreview = ({
  ...
  return (
    <div className={`w-full flex items-center gap-2 p-2 rounded-md cursor-pointer group ${className}`} onClick={onClick}>
      {isImage ? (
        <PhotoProvider>
          <PhotoView src={getPlanIncEndpoint(`${item.path}?token=${RootStore.Get(UserStore).tokenData.value?.token}`)}>
            <div>
      ...
      <div className="flex-1 min-w-0">
          ...
          {isS3File && showExtraInfo && (
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>{t('cloud-file')}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
          {showAssociationIcon && !item.noteId && showExtraInfo && (
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>{t('no-note-associated')}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
        </div>
        {showExtraInfo && (
          <div className="text-xs text-gray-500 flex items-center gap-2 my-1">
            <span className="rounded-md px-1.5 py-0.5 bg-muted text-muted-foreground">{fileNameAndExt.ext}</span>
...
const getCardClassName = (isDragging?: boolean, isDraggingOver?: boolean) => {
  const baseClasses = 'mb-2 p-4 hover:bg-muted !transition-all duration-200';
...
const ResourceCard = observer(({ item, isSelected, onSelect, isDragging, isDraggingOver }: ResourceCardProps) => {
  ...
  if (item.isFolder) {
    return (
      <Card {...cardProps} className="border-none shadow-none">
  ...
  return (
    <Card {...cardProps} className="border-none shadow-none">
      <div className="flex items-center gap-4">
        <Checkbox checked={isSelected} onCheckedChange={() => onSelect(item.id!)} className="z-10" />
