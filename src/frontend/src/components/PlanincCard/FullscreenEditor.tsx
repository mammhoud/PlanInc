...
export const FullscreenEditor = observer(({ planincItem, isOpen, onClose }: FullscreenEditorProps) => {
  ...
  const handleClose = () => {
    planinc.fullscreenEditorNoteId = null;
  ...
  useEffect(() => {
    if (isOpen && editorMode === 'edit') {
      const originalMode = localStorage.getItem('planinc-editor-view-mode');
      localStorage.setItem('planinc-editor-view-mode', 'wysiwyg');
      setViewMode('wysiwyg');
      ...
      return () => {
        if (originalMode) {
          localStorage.setItem('planinc-editor-view-mode', originalMode);
        } else {
          localStorage.removeItem('planinc-editor-view-mode');
        }
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (planincItem.id) {
        planinc.noteDetail.call({ id: planincItem.id }).then(() => {
          if (planinc.noteDetail.value) {
            planinc.curSelectedNote = _.cloneDeep(planinc.noteDetail.value);
          }
        });
      } else {
        // Fallback to prop data if no id
        planinc.curSelectedNote = _.cloneDeep(planincItem);
        planinc.noteDetail.value = _.cloneDeep(planincItem);
      }
    }
  }, [isOpen, planincItem.id]);
  ...
  useEffect(() => {
    ...
    const mobileHeader = document.querySelector('.planinc-mobile-header') as HTMLElement;
    const bottomBar = document.querySelector('.planinc-bottom-bar') as HTMLElement;
  ...
  const handleEditorSended = async () => {
    ...
    if (planincItem.id) {
      ...
      await planinc.noteDetail.call({ id: planincItem.id });
      if (planinc.noteDetail.value) {
        planinc.curSelectedNote = _.cloneDeep(planinc.noteDetail.value);
      }
    }
  ...
  const isLongText = (planincItem?.content?.length ?? 0) > 1000;
      ...
      <div className="h-full flex">
        ...
        >
          ...
          {isPc && (
            <div className="flex items-center justify-between py-4 flex-shrink-0 border-b border-border">
              ...
              <div className="flex-1 flex justify-end ml-2 gap-2">
                ...
                {editorMode === 'edit' && (
                  <div id={`editor-top-toolbar-${planincItem.id}`} className="flex justify-end"></div>
                )}
              </div>
            </div>
          )}
          ...
          {editorMode === 'preview' ? (
            ...
            >
              <MarkdownRender
                content={planinc.noteDetail.value?.content ?? planincItem.content}
                onChange={(newContent) => {
                ...
                largeSpacing={true}
              />
              <ReferencesContent planincItem={planinc.noteDetail.value ?? planincItem} className="my-4" />
              <div className={planincItem.attachments?.length != 0 ? 'my-2' : ''}>
                <FilesAttachmentRender files={planinc.noteDetail.value?.attachments ?? planincItem.attachments ?? []} preview />
              </div>
            </div>
          ) : (
            /* Edit mode - render with PlanIncEditor */
            <div 
