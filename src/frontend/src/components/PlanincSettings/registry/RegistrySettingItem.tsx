    ...
    case 'switch':
      return (
        <Item
          ...
          rightContent={
            <Switch isSelected={!!value} onChange={(e) => write(e.target.checked)} />
          }
        />
      );
          ...
          rightContent={
            <div className="flex items-center gap-3 w-48">
              <Slider
                ...
                onChange={(next) => setDraft(String(Array.isArray(next) ? next[0] : next))}
                onChangeEnd={(next) => void write(Array.isArray(next) ? next[0] : next)}
