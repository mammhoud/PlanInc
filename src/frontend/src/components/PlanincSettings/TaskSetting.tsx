      ...
      <Item
        ...
        rightContent={
          <div className="flex gap-4">
            ...
            />
            <Switch
              thumbIcon={planinc.updateArchiveTask.loading.value ? <Icon icon="eos-icons:three-dots-loading" width="24" height="24" /> : null}
              isDisabled={planinc.updateArchiveTask.loading.value}
              isSelected={planinc.ArchiveTask?.isRunning}
              onChange={async e => {
                await planinc.updateArchiveTask.call(e.target.checked)
              }}
            />
          </div>} />
      <AITasksPanel />
...
const TasksPanel = observer(() => {
  const { t } = useTranslation()
  const planinc = RootStore.Get(PlanIncStore)
  return <> {planinc.task.value && <Table shadow="none" className="mb-2">
    <TableHeader>
      <TableColumn>{t('name-db')}</TableColumn>
      <TableColumn>{t('schedule')}</TableColumn>
      <TableColumn>{t('last-run')}</TableColumn>
