...
import { PlanIncCard, PlanIncItem } from "../PlanIncCard";
              ...
              !!metadata?.notes?.length && (
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <Icon icon="hugeicons:file-02" width="14" height="14" />
                      <span className="text-xs">
                        {i18n.t('reference-notes', { count: metadata.notes.length })} ({metadata.notes.length})
                      </span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent side="bottom" align="start" className="overflow-y-auto">
                    <ScrollArea className="flex flex-col gap-2 p-2 h-[400px]" onBottom={() => { }}>
                      {
                        //@ts-ignore
                        metadata?.notes?.map((item: PlanIncItem, index: number) => (
                          <PlanIncCard key={item.id || index} className='w-[300px] md:w-[600px]' planincItem={item!} withoutHoverAnimation />
                        ))
                      }
                    </ScrollArea>
                  </PopoverContent>
          ...
          </div>
          {
            !shareMode && !isStreaming && (
              <div className={`${isMobile ? 'opacity-70' : 'opacity-0 group-hover:opacity-100'} transition-opacity duration-200 mb-4`}>
                <div className="flex gap-2 backdrop-blur-sm rounded-full p-1 items-center">
                  <IconButton
                    tooltip={i18n.t('add-to-planinc')}
                    icon="basil:lightning-solid"
                    classNames={{
                      icon: 'text-yellow-500'
                    }}
                    onClick={() => {
                      RootStore.Get(PlanIncStore).upsertNote.call({
                        content: content,
                        type: NoteType.PLANINC,
                      })
                    }}
                    size={20}
                    containerSize={25}
                    ...
                    onClick={async () => {
                      ...
                      if (aiStore.currentConversation.value) {
                        try {
                          ...
                          const shareData = `planinc-ai-share-${conversationId}`;
                          const encodedId = btoa(shareData);
                          const shareUrl = getPlanIncEndpoint(`/ai-share/${encodedId}`)
                          copy(shareUrl);
...
export const PlanIncChatBox = observer(({ shareMode = false }: { shareMode?: boolean } = {}) => {
  const aiStore = RootStore.Get(AiStore)
