...
import { UserAvatar } from "@/components/PlanIncCard/commentButton";
import { PlanIncStore } from "@/store/planincStore";
import { DialogStore } from "@/store/module/Dialog";
import { PlanIncFollowDialog, PlanIncFollowingDialog } from "@/components/PlanIncFollowDialog";
                  ...
                  <Button
                    onClick={() => {
                      RootStore.Get(DialogStore).setData({
                        ...
                        title: t('follow'),
                        size: 'lg',
                        content: <PlanIncFollowDialog onConfirm={() => {
                          store.loadAllData()
                        }} />,
                      })
                    }}
          ...
          <div className="flex items-center gap-8 text-sm text-desc">
            <div className="flex items-center gap-1 cursor-pointer" onClick={() => {
              RootStore.Get(DialogStore).setData({
                isOpen: true,
                title: t('following'),
                content: <PlanIncFollowingDialog isFollowing={false} data={store.followList.value ?? []} onConfirm={() => {
                  store.loadAllData()
                }} />,
              })
            }}>
            ...
            <div className="flex items-center gap-1 cursor-pointer"
              onClick={() => {
                RootStore.Get(DialogStore).setData({
                  isOpen: true,
                  title: t('following'),
                  content: <PlanIncFollowingDialog isFollowing={true} data={store.followingList.value ?? []} onConfirm={() => {
                    store.loadAllData()
                  }} />,
                })
              }}>
