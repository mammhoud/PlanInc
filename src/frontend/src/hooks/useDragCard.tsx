...
export const useDragCard = ({ notes, onNotesUpdate, activeId, setActiveId, insertPosition, setInsertPosition, isDragForbidden, setIsDragForbidden }: UseDragCardProps) => {
  ...
  const shouldEnableDrag = planinc.fullscreenEditorNoteId === null;
  ...
  const handleDragStart = (event: any) => {
    ...
    const planinc = RootStore.Get(PlanIncStore);
    if (planinc.fullscreenEditorNoteId !== null) {
...
interface DraggablePlanIncCardProps {
  planincItem: any;
...
export const DraggablePlanIncCard = ({ planincItem, showInsertLine, insertPosition, isDragForbidden }: DraggablePlanIncCardProps) => {
  const { t } = useTranslation()
  
  const { setNodeRef: setDroppableRef, isOver } = useDroppable({
    id: `drop-${planincItem.id}`,
  });
  ...
  return (
    <div className="relative">
      {showInsertLine && insertPosition === 'top' && (
        <div className={`absolute -top-2 left-0 right-0 h-1 z-50 rounded-full ${isDragForbidden ? 'bg-destructive' : 'bg-info'}`} />
      )}
      ...
      <div
        ref={setDroppableRef}
        className={`relative
          ${isDragging ? 'bg-muted border-2 border-dashed border-border rounded-lg' : ''}
          ${isOver && isDragForbidden ? 'border-2 border-dashed !border-destructive rounded-lg' : ''}
        `}
      >
        {/* Forbidden icon overlay when hovering over wrong area */}
        {isOver && isDragForbidden && !isDragging && (
          <div className="absolute inset-0 flex items-center justify-center z-50 bg-destructive/10 rounded-lg">
        ...
        {isDragging ? (
          <div className="flex items-center justify-center p-8 min-h-[100px]">
            <div className="text-ignore text-center">
          ...
          <div
            ref={setDraggableRef}
            style={dragStyle}
            {...(planincItem.isExpand ? {} : attributes)}
            {...(planincItem.isExpand ? {} : listeners)}
            className="cursor-default!"
          >
            <PlanIncCard planincItem={planincItem} />
          </div>
        )}
      </div>

      {showInsertLine && insertPosition === 'bottom' && (
        <div className={`absolute -bottom-2 left-0 right-0 h-1 z-50 rounded-full ${isDragForbidden ? 'bg-destructive' : 'bg-info'}`} />
      )}
    </div>
  );
};