...
class PlanInc {
    ...
    private fun saveThemePreference(activity: Activity, isDarkMode: Boolean, hexColor: String) {
        try {
            val sharedPref = activity.getSharedPreferences("planinc_theme", Context.MODE_PRIVATE)
            with(sharedPref.edit()) {
                putBoolean("dark_mode", isDarkMode)
                putString("last_color", hexColor)
                apply()
            }
            Log.i("PlanInc", "Theme preference saved: isDark=$isDarkMode, color=$hexColor")
        } catch (e: Exception) {
            Log.e("PlanInc", "Error saving theme preference: ${e.message}")
        }
    }
    ...
    fun getSavedTheme(activity: Activity): Map<String, Any> {
        return try {
            ...
            Log.e("PlanInc", "Error getting saved theme: ${e.message}")
            mapOf(
                "isDarkMode" to false,
                "lastColor" to "#FFFFFF"
            )
    ...
    fun applyStartupTheme(activity: Activity) {
        try {
            ...
            Log.i("PlanInc", "Startup theme applied: $lastColor")
        } catch (e: Exception) {
            Log.e("PlanInc", "Error applying startup theme: ${e.message}")
        }
    }
    ...
    fun openAppSettings(activity: Activity) {
        try {
            ...
            try {
                ...
                Log.e("PlanInc", "Failed to open settings: ${fallbackException.message}")
            }
        }
    }
}
