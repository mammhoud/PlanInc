...
class MainActivity : TauriActivity() {
        ...
        window.decorView.postDelayed({
            try {
                findWebView(window.decorView)?.let { webView ->
                    ...
                    Log.i("PlanIncApp", "WebView bounce effect enabled")
                }
            } catch (e: Exception) {
                Log.e("PlanIncApp", "Failed to enable WebView bounce effect: ${e.message}")
            }
    ...
    private fun handleShortcutIntent() {
        ...
        intent?.data?.let { uri ->
            if (uri.scheme == "planinc" && uri.host == "shortcut") {
