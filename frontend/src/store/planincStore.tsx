...
import { Attachment, type Note } from '@/lib/apiTypes';
...
export class PlanIncStore implements Store {
  sid = 'PlanIncStore';
  noteContent = '';
  ...
  noteTypeDefault: NoteType = NoteType.PLANINC
  ...
  planincList = new PromisePageState({
    function: async ({ page, size }) => {
      return this.getFilteredNotes({
        ...
        offlineFilter: (note: OfflineNote) => {
          return Boolean(note.type === NoteType.PLANINC && !note.isArchived && !note.isRecycle);
        }
      });
    }
  })
  ...
  updateArchiveTask = new PromiseState({
    function: async (isStart) => {
      if (isStart) {
        await api.task.upsertTask.mutate({ type: 'start', task: ARCHIVE_PLANINC_TASK_NAME })
      } else {
        await api.task.upsertTask.mutate({ type: 'stop', task: ARCHIVE_PLANINC_TASK_NAME })
      }
      await this.task.call()
    }
  })
  ...
  get ArchiveTask() {
    return this.task.value?.find(i => i.name == ARCHIVE_PLANINC_TASK_NAME)
  }
