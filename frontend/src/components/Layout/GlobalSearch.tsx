...
import { ConvertTypeButton } from '../PlanIncCard/cardFooter';
...
export const GlobalSearch = observer(({ isOpen, onOpenChange }: GlobalSearchProps) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    setSearchQuery(value: string) {
      ...
      if (value) {
        ...
        planincStore.resourceList.resetAndCall({
          page: 1,
          size: 20,
          searchText: '',
          folder: undefined,
  ...
  useEffect(() => {
    if (isOpen) {
      store.searchQuery = planincStore.searchText || '';
      if (searchInputRef.current) {
        searchInputRef.current.focus();
      }
    }
  ...
  useEffect(() => {
    planincStore.noteListFilterConfig.isUseAiQuery = store.isAiQuestion;
  }, [store.isAiQuestion]);
  ...
  const debouncedSearch = useRef(
    _.debounce(async (query) => {
      ...
      planincStore.searchText = query;
      planincStore.globalSearchTerm = query;
      ...
      try {
        ...
        planincStore.noteListFilterConfig.isUseAiQuery = isAiQuery;
        ...
        const notes = await planincStore.noteList.resetAndCall({ page: 1, size: 20, type: -1, isArchived: null });
        // await planincStore.planincList.resetAndCall({ page: 1, size: 20 });
        // 3. Search for resources using the API
        const resources = await planincStore.resourceList.resetAndCall({
          page: 1,
          size: 20,
  ...
  const navigateToResource = (resource: ResourceType) => {
    //download
    downloadFromLink(getPlanIncEndpoint(resource.path));
    onOpenChange(false);
  };
        ...
        <ConvertTypeButton
          ...
          tooltip={
            <div className="max-w-[400px] p-0 rounded-2xl bg-transparent">
              <PlanIncCard planincItem={note} withoutHoverAnimation withoutBoxShadow className='!border-none' />
            </div>
          }
        />
      </div>
