    ...
    <ScrollArea
      ...
      {aiStore.conversactionList.value?.map(item => (
        ...
        >
          ...
          {/* Was `isMobile ? flex : hidden group-hover:flex`: rename/delete were
              hidden until hover, which a touch *tablet* (where isMobile is false)
              can never trigger. `.hover-only-on-fine` keys off the pointer, so it
              works on every touch size and still reveals on mouse hover. */}
          <div data-reveal className={`flex hover-only-on-fine items-center gap-1 absolute right-2 top-1/2 -translate-y-1/2 px-2 rounded`}>
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
