...
export const PerferSetting = observer(() => {
  ...
  >
    ...
    <Item
      ...
      rightContent={<Switch
        checked={planinc.config.value?.isHiddenNotification}
    ...
    <Item
      ...
      rightContent={<Switch
        checked={planinc.config.value?.isHiddenMobileBar}
        onCheckedChange={(checked) => {
          PromiseCall(api.config.update.mutate({
    ...
    <Item
      ...
      rightContent={<Switch
        checked={planinc.config.value?.isHideCommentInCard}
        onCheckedChange={(checked) => {
          PromiseCall(api.config.update.mutate({
    ...
    <Item
      ...
      rightContent={<Switch
        checked={planinc.config.value?.isOrderByCreateTime}
        onCheckedChange={(checked) => {
          PromiseCall(api.config.update.mutate({
    ...
    <Item
      ...
      rightContent={
        <Switch
          checked={planinc.config.value?.isCloseDailyReview}
          onCheckedChange={(checked) => {
            PromiseCall(api.config.update.mutate({
    ...
    <Item
      ...
      rightContent={<div className="flex flex-wrap gap-2 w-full min-w-0 sm:justify-end">
        <SelectDropdown
    ...
    <Item
      ...
      rightContent={
        <Switch
          checked={planinc.config.value?.isUsePlanIncHub}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="inline-flex">
              <Switch
                checked={planinc.config.value?.isCloseBackgroundAnimation}
                onCheckedChange={(checked) => {
                  PromiseCall(api.config.update.mutate({
          ...
          <TooltipContent>
            <GradientBackground className="rounded-lg w-[200px] h-[100px]">
              <div ></div>
            </GradientBackground>
          </TooltipContent>
        </Tooltip>
...

    {
      user.isSuperAdmin && (
        <Item
          ...
          rightContent={<Input
            className="w-full min-w-0 sm:max-w-[400px] sm:ml-auto"
...

    {
      user.isSuperAdmin && (
        <Item
          ...
          rightContent={
            <Switch
              checked={planinc.config.value?.signinFooterEnabled ?? false}
              onCheckedChange={async (checked) => {
                await PromiseCall(api.config.update.mutate({
