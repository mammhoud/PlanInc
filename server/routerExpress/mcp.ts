...
import { searchPlanIncTool } from '@server/aiServer/tools/searchPlaninc';
import { upsertPlanIncTool } from '@server/aiServer/tools/createPlaninc';
import { updatePlanIncTool } from '@server/aiServer/tools/updatePlaninc';
import { deletePlanIncTool } from '@server/aiServer/tools/deletePlaninc';
...
function createPlanIncMcpServer(getAuthContext: () => AuthContext) {
  const server = new McpServer({
    name: 'planinc-mcp-server',
    version: '1.0.0',
  });
  ...
  createToolWithContext('searchPlanInc', searchPlanIncTool);
  createToolWithContext('upsertPlanInc', upsertPlanIncTool);
  createToolWithContext('updatePlanInc', updatePlanIncTool);
  createToolWithContext('deletePlanInc', deletePlanIncTool);
...
async function createStreamableTransport(authContext: AuthContext): Promise<StreamableHTTPServerTransport> {
  ...
  const server = createPlanIncMcpServer(() => session.authContext);
