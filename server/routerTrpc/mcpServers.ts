...
export const mcpServersRouter = router({
  ...
  list: authProcedure
    ...
    .query(async () => {
      const servers = await db.mcpServers.findMany({
  ...
  get: authProcedure
    ...
    .query(async ({ input }) => {
      const server = await db.mcpServers.findUnique({
        where: { id: input.id }
      });
  ...
  update: authProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const existingServer = await db.mcpServers.findUnique({
        where: { id }
      });
      ...
      const server = await db.mcpServers.update({
        where: { id },
        data: {
