...
import { z } from 'zod';
import { db } from '../db';
import { commentsSchema, accountsSchema, NotificationType } from '@shared/lib/recordSchemas';
...
async function checkNoteAccess(noteId: number, ctx: any): Promise<boolean> {
  ...
  const internalShare = await db.noteInternalShare.findFirst({
    where: {
      noteId: noteId,
      accountId: userId
    }
...
export const commentRouter = router({
  create: publicProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      if (parentId) {
        const parentComment = await db.comments.findFirst({
      ...
      if (content.includes('@PlanInc AI')) {
