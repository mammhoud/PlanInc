...
import { aiProviders, aiModels } from '@shared/lib/recordSchemas';
...
export const aiRouter = router({
  embeddingUpsert: authProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const createTime = await db.notes.findUnique({ where: { id } }).then(i => i?.createdAt)
  ...
  testConnect: authProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const provider = await db.aiProviders.findUnique({
          where: { id: providerId }
        });
  ...
  createProvider: authProcedure
    ...
    .mutation(async ({ input }) => {
      return await db.aiProviders.create({
        data: input,
        include: { models: true }
      });
    }),
  ...
  updateProvider: authProcedure
    ...
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      return await db.aiProviders.update({
        where: { id },
        data,
        include: { models: true }
      });
  ...
  deleteProvider: authProcedure
    ...
    .mutation(async ({ input }) => {
      return await db.aiProviders.delete({
        where: { id: input.id }
      });
    }),
  ...
  createModel: authProcedure
    ...
    .mutation(async ({ input }) => {
      return await db.aiModels.create({
        data: input,
        include: { provider: true }
      });
    }),
  ...
  updateModel: authProcedure
    ...
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      return await db.aiModels.update({
        where: { id },
        data,
        include: { provider: true }
      });
  ...
  deleteModel: authProcedure
    ...
    .mutation(async ({ input }) => {
      return await db.aiModels.delete({
        where: { id: input.id }
      });
    }),
  ...
  createModelsFromProvider: authProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      for (const model of models) {
        const created = await db.aiModels.create({
          data: {
            providerId,
            title: model.name,
            modelKey: model.id,
