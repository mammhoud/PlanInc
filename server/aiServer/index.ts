...
import { NotificationType } from '@shared/lib/recordSchemas';
...
export class AiService {
  ...
  static async embeddingDeleteAll(id: number, VectorStore: LibSQLVector) {
    await VectorStore.truncateIndex({ indexName: 'planinc' });
  }
  ...
  static async embeddingUpsert({ id, content, type, createTime, updatedAt }: { id: number; content: string; type: 'update' | 'insert'; createTime: Date; updatedAt?: Date }) {
    try {
      ...
      if (config.excludeEmbeddingTagId) {
        const tag = await db.tag.findUnique({ where: { id: config.excludeEmbeddingTagId } });
  ...
  static async completions({
    ...
    try {
      ...
      // See https://github.com/mammhoud/planinc/issues/1122
...
If you need to update the note, use the updatePlanIncTool.
If you need to create a new note, use the upsertPlanIncTool.
Remember: ALWAYS use tools to implement your suggestions rather than just describing what should be done.`,
  ...
  static async transcribeAudio({
    ...
    try {
      ...
      const voiceModel = await db.aiModels.findUnique({
