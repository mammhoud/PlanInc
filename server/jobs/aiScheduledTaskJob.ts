...
export class AIScheduledTaskJob {
  ...
  static async initialize(): Promise<void> {
    ...
    await boss.work(AI_SCHEDULED_TASK_QUEUE, { batchSize: 1 }, async (jobs: any[]) => {
      ...
      try {
        ...
        const task = await db.aiScheduledTask.findUnique({
          where: { id: taskId }
        });
        ...
        await db.aiScheduledTask.update({
          where: { id: taskId },
          data: {
            lastRun: new Date(),
            lastResult: {
  ...
  static async scheduleAllTasks(): Promise<void> {
    ...
    const tasks = await db.aiScheduledTask.findMany({
  ...
  static async createTask(params: {
    ...
    const task = await db.aiScheduledTask.create({
      data: {
        name,
        prompt,
        schedule,
  ...
  static async deleteTask(taskId: number, accountId: number): Promise<boolean> {
    // Verify ownership
    const task = await db.aiScheduledTask.findFirst({
    ...
    await db.aiScheduledTask.delete({
      where: { id: taskId }
    });
  ...
  static async getTasks(accountId: number) {
    return db.aiScheduledTask.findMany({
      where: { accountId },
