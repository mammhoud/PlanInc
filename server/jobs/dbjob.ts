...
export class DBJob extends BaseScheduleJob {
  ...
  protected static async RunTask() {
    try {
      ...
      const targetFile = UPLOAD_FILE_PATH + `/planinc_export.bko`;
      ...
      if (config.objectStorage === 's3') {
        ...
        await s3ClientInstance.send(new PutObjectCommand({
          ...
          Key: `/PLANINC_BACKUP/planinc_export.bko`,
          Body: fileStream
        }));
        result = {
          filePath: `/api/s3file/PLANINC_BACKUP/planinc_export.bko`,
          progress: finalProgress
        };
      } else {
        result = {
          filePath: `/api/file/planinc_export.bko`,
          progress: finalProgress
        };
      }
