...
import { attachmentsSchema, tagSchema, tagsToNoteSchema, notesSchema } from "@shared/lib/recordSchemas";
import { z } from "zod";
...
export class RecommandJob extends BaseScheduleJob {
  ...
  static async initialize(): Promise<void> {
    try {
      ...
      if (followCount > 0) {
        ...
        // Check if task is already scheduled via job queue
  ...
  protected static async RunTask() {
    ...
    try {
      ...
      const hasCache = await db.cache.findFirst({
        where: { key: 'recommand_list' },
        select: { id: true }
      });
