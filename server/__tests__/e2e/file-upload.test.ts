 ...
 *   - SurrealDB reachable (http://surrealdb:8000 — no SQL database involved)
...
beforeAll(async () => {
  ...
  // Read the JWT secret through the SurrealDB data layer — the only database
  // client in the stack (server/db.ts keeps the document-style
  // `db.<table>.<method>()` call shapes).
  const { db } = await import('../../db');
  try {
    const config = await db.config.findFirst({ where: { key: 'JWT_SECRET' } });
